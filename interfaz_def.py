...
def obtener():
	...
	if (value_inside.get() == "SNMP-V2"):
		...
		sys_value.set("Selecciona un objecto")
	...
	button_set= Button(app, text="Set", command = lambda:set_fun(parametro))
	button_get=Button(app, text="Get", command = lambda:get_handler(session, parametro))
	button_getNext=Button(app, text="Get next", command = lambda:getnext_fun(parametro))
	button_set.place(x=200, y=550)
	button_get.place(x=250, y =550)
	button_getNext.place(x=300, y =550)
